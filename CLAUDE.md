...
- **Cloudflare (premium)** — `playCloudflareChunk()` POSTs each chunk to the Worker and plays the returned MP3 via an `<audio>` element. Has an in-memory audio cache with next-chunk prefetch, and supports live `playbackRate` changes. On quota/capacity errors (the Worker returns 429 + a `detail` string) `quotaFallback()` degrades tiers automatically — Aura-2 → Aura-1 → MeloTTS → free browser voice — re-chunking by character position so the listening spot survives the engine switch.
